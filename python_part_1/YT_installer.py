...
def download():
    ...
    try:
        ...
        if not save_path:
            messagebox.showinfo('Отменено', "Загрузка видео отменена пользователем")
            return
        
        video.download(output_path=download_dir, filename=download_name)
...
def audio_download():
    ...
    try:
        ...
        if not save_path:
            messagebox.showinfo('Отменено', "Загрузка аудио отменена пользователем")
            return

        audio.download(output_path=download_dir, filename=download_name)
